...
import org.pathvisio.io.gpml2021.GPML2021FormatAbstract;
import org.pathvisio.io.gpml2021.GPML2021FormatAbstract.AttributeInfo;
...
class GpmlFormat200X extends GPML2021FormatAbstract implements GpmlFormatReader {
	...
	public static final GpmlFormatReader GPML_2008A = new GpmlFormat200X("GPML2008a.xsd",
			Namespace.getNamespace("http://genmapp.org/GPML/2008a"));
	...
	private void mapDeprecatedAttribute(PathwayElement o, Element el, String key, String tag, String name)
		...
		if (result != null)
			o.setDynamicProperty(key, result);
	}
	...
	protected void mapLabelData(PathwayElement o, Element e) throws ConverterException {
		...
		if ("None".equals(outline))
			o.setShapeType(ShapeType.NONE);
		else if ("Rectangle".equals(outline))
			o.setShapeType(ShapeType.RECTANGLE);
		else if ("RoundedRectangle".equals(outline))
			o.setShapeType(ShapeType.ROUNDED_RECTANGLE);
	...
	protected void mapLineData(PathwayElement o, Element e) throws ConverterException {
		...
		for (int i = 0; i < pointElements.size(); i++) {
			...
			MPoint mp = o.new MPoint(Double.parseDouble(getAttribute("Line.Graphics.Point", "x", pe)) / CONVERSION,
			...
			if (i == 0) {
				...
				/**
				 * read deprecated Head attribute for backwards compatibility. If an arrowhead
				 * attribute is present on the other point, it overrides this one.
				 */
	 ...
	 * 
	 */
	private static final Namespace BP_LEV2 = Namespace.getNamespace("bp",
