...
public interface ConnectorShape {
	 ...
	 * 
	 * @param startLineEndingWidth.
	 * @param endLineEndingWidth.
	 */
	...
	/**
	 * Checks whether the waypoints as provided by the ConnectorRestrictions are
	 * valid and will be used to draw the connector path
	 * 
	...
	/**
	 * The class for a waypoint, a point through which the connector passes. Each
	 * waypoint will have a handle in the view, so the user can modify it's
	 * position.
	 * 
	 * @author unknown.
	 */
	...
	/**
	 * A single segment of the connector path. This is simply a combination of a
	 * start and end Point2D.
	 */
	public class Segment {
		...
		public Point2D getMCenter() {
			return new Point2D.Double(start.getX() + (end.getX() - start.getX()) / 2,
		...
		/**
		 * The coordinates of the new end point if the length of the segment is reduced
		 * by the parameter.
		 */
		...
		/**
		 * The coordinates of the new start point if the length of the segment is
		 * reduced by the parameter. Easier to use the NewEndPoint method than rewrite
		 * it CAUTION:: may need examining with curved lines, The Math may not hold Also
		 * caution for elbow lines where the last line may be smaller than the width of
		 * the arrow shape We may have to put this function in each individual Connector
